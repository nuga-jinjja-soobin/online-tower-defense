...
export const packetParser = (buffer) => {
  // 1. 패킷 타입 길이만큼 버퍼 읽을 위치 지정
  let offset = 0;

  // 2. 패킷 타입 - 핸들러 (2 bytes)
  const packetType = buffer.readUInt16BE(offset);
  ...
  const version = buffer.slice(offset, offset + versionLength).toString('utf-8');
  ...
  console.log(`디코딩한 패킷 값: ${JSON.stringify(payloadData)}`);

...
export const gameEndHandler = async ({ socket, payload }) => {
  ...
  else
  {
    ...
    DatabaseManager.GetInstance().updateHighScore(gameEndUser.id, findGame.gameData[gameEndUser.id].score);  
    DatabaseManager.GetInstance().updateHighScore(opponentUser.id, findGame.gameData[opponentUser.id].score);  
